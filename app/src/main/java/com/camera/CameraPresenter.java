...
//        Core.flip(trackingInfo.matBgr, trackingInfo.matBgr, 1);
    ...
    private Handler faceTrackingHandler = new Handler(faceTrackThread.getLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            System.out.println("测试我来了  faceTrackingHandler length " + imageData.width + "," + imageData.height + "," + imageData.channels + "," + Arrays.toString(imageData.data));
            ...
            if (null != faces && faces.length != 0) {
                System.out.println("测试我来了 $faceTrackingHandler length "+faces.length);
                int maxIndex = 0;
                double maxWidth = 0;
                for (int i = 0; i < faces.length; ++i) {
                    System.out.println("测试我来了 faceTrackingHandler width "+faces[i].position.width);
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    ...
                    public void run() {
                        i++;
                        if(i%100==0){
                            System.out.println("测试我来了 $faceTrackingHandler null");
                        }
                    }
                });
            }
    ...
    private Handler faceRecognizerHandler = new Handler(faceRecognizerThread.getLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            if (cameraFragment.needFaceRegister) {
                ...
                System.out.println("测试我来了  开启人脸注册 if "+trackingInfo.faceInfo.width );
                ...
                System.out.println("测试我来了  开启人脸注册 "+canRegister);
                //进行人脸的注册
                if (canRegister) {
                    //注册人脸
            ...
            System.out.println("测试我来了  进行人脸识别 "+trackingInfo.faceInfo.width);
            //进行人脸识别
            if (trackingInfo.faceInfo.width != 0) {
                //特征点检测
                pointFs = new SeetaPointF[5];
                ...
                System.out.println("测试我来了  进行人脸识别 faceCount"+faceCount);
                if (faceCount > 0) {
                    //不空进行特征提取，并比对
                    float[] feats = new float[faceRecognizer.getExtractFeatureSize()];
                    faceRecognizer.extract(imageData, pointFs, feats);
                    FaceEntity faceEntity = faceHelper.searchFace(feats);
                    System.out.println("测试我来了  进行人脸识别 faceEntity"+faceEntity);
                    if (null != faceEntity) {
